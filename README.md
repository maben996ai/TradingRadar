...
TradingRader 是一个面向港美股投资者的金融内容聚合工具，用于把 X/Twitter、YouTube 等信源的更新集中到统一的信息流里，并围绕订阅管理、内容抓取、通知和后续投研分析能力持续迭代。
...
- 当前可解析并抓取的真实信源：X/Twitter、YouTube
...
- 当前真实采集器注册了 X/Twitter 和 YouTube；Bilibili 等平台在模型和部分测试数据中保留，但尚未作为当前解析入口开放
- 金融时讯、市场宏观、财经日历和内容分析页面仍有占位能力，后续需要接入真实数据源或分析服务
